...
```{r, adding additional codes manually, echo=FALSE}
# df_rq2_ <- df_rq2_ %>%
...
df_rq2_ <- df_rq2_ %>%
  mutate(cat = case_when(
    !is.na(cat) ~ cat,  # Wenn `cat` nicht NA ist, behalte den ursprünglichen Wert
    str_detect(learn_descr, "Keine") ~ "Nichts"  # Setze "Nichts" wenn "Keine" in `learn_descr` gefunden wird
  ))
...
```{r cross table: lo x act, echo=FALSE}
...
rq2_crosstable <- as.data.frame(table(df_rq2_test_$act_no, 
                                      df_rq2_test_$cat))
